...
class UploadThreadManager(val activity: AppCompatActivity) {
    ...
    init {
        val intent = Intent(activity, UploadMultipleFilesService::class.java)
    ...
    fun startUploadService(
        ...
        L.print(this, "Start uploading service - $fileTypes")
    ...
    fun testHasUploads(success: () -> Unit) {
        ...
        L.print(this,"property item counter - ${property.itemCounter} size - ${property.fileTypes.size - 1} is uploading - ${property.isUploading}")
        if (property.itemCounter < property.fileTypes.size - 1
